...
<div style="width: 600px; margin: 50px auto; background-color: #ffffff; padding: 30px; border-radius: 8px; box-shadow: 0 0 10px rgba(0,0,0,0.1);">
    <h2 style="margin-bottom: 20px; color: #333;">Alasan Penolakan</h2>
    ...
    <form method="POST" action="{{ route('peminjaman.tolak', $peminjaman->id) }}">
        @csrf
        @method('PUT')

        <div style="margin-bottom: 15px;">
            <label style="display: block; font-weight: bold; margin-bottom: 5px;">Alasan Ditolak</label>
            <input type="text" name="alasan_ditolak" required value="{{ old('alasan_ditolak') }}"
